...
export class BockTodoComponent {
  ...
  editTitle(event: [number,string]){
    ...
    if(this.new_list){
      ...
      const index = this.block_todo_list.findIndex(todos_list => todos_list.id === event[0]);
  ...
  countCompletedItems() {
    this.actual_completed_item = this.actual_list.todos_list.filter((todo) => todo.active).length;
  }
