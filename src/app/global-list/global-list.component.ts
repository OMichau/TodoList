...
export class GlobalListComponent {
  ...
  addToList(event: Todo) {
    ...
    this.changementEmitter.emit([this.block_todo.id,this.block_todo.todos_list]);
  }
  ...
  activeTodo(id: number) {
    const temp = this.block_todo.todos_list.findIndex((todo) => todo.id == id);
    this.block_todo.todos_list[temp].active = !this.block_todo.todos_list[temp].active;
    this.changementEmitter.emit([this.block_todo.id,this.block_todo.todos_list]);
  }
  ...
  changeNameTodo(obj: [string, number]) {
    const temp = this.block_todo.todos_list.findIndex((todo) => todo.id == obj[1]);
